...
async function runCli(prompt, model, requestId, assignmentId) {
  const child = spawn("codex", cliArgs(model), {
    cwd: process.cwd(),
    env: process.env,
  ...
  stdout.on("line", (line) => {
    ...
    if (
      ...
      cliError = boundedText(event.message, 2048) || "Codex execution failed.";
    }
  });
          ...
          : [
              {
                ...
                message:
                  "Sign in to Codex on this computer, then validate again.",
              },
            ],
      });
      break;
