...
export const CLAUDE_PLUGIN_ID = "conclave.claude-code";
export const CLAUDE_PLUGIN_VERSION = "1.0.0";

export const claudeWorkerManifest: WorkerPluginManifest = {
  ...
  digest: "sha256-conclave-claude-plugin-v1-digest",
};

export interface ClaudeSpawnChild {
  readonly pid?: number;
  readonly stdout: {
...
export function buildClaudePrompt(input: WorkerPluginInput): string {
  return [
    "You are a Conclave Claude Code implementation and review agent.",
    ...
    "The payload must be a valid Conclave protocol result. Describe proposed file operations explicitly.",
    `Objective:\n${input.objective}`,
    `Assigned Role:\n${input.role}`,
...
export async function executeClaudeWorker(
  input: WorkerPluginInput,
  context: WorkerPluginContext,
  customSpawner?: ClaudeSpawner,
): Promise<WorkerPluginOutput> {
  const startedAt = new Date().toISOString();
  const config = context.config ?? {};
  const executable =
    typeof config.executable === "string" ? config.executable : "claude";
  ...
  context.log(
    "info",
    `Starting Claude Code CLI worker execution for role '${input.role}'`,
    {
  ...
  context.progress("init", 10, "Preparing Claude Code CLI execution context");

  const prompt = buildClaudePrompt(input);
  const spawnFn =
    customSpawner ?? (defaultClaudeSpawn as unknown as ClaudeSpawner);
  ...
  return new Promise<WorkerPluginOutput>((resolve) => {
    ...
    const timer = setTimeout(() => {
      timedOut = true;
      context.log("warn", `Claude execution timed out after ${timeoutMs}ms`);
      terminate();
    }, timeoutMs);

    const onAbort = () => {
      cancelled = true;
      context.log("warn", "Claude execution cancelled via AbortSignal");
      terminate();
    };
    context.signal.addEventListener("abort", onAbort, { once: true });

    let child: ClaudeSpawnChild | undefined;
    try {
      context.progress(
        "executing_cli",
        30,
        `Spawning Claude Code CLI (${executable})`,
      );
      ...
      child.on("error", (error: Error) => {
        context.log("error", `Claude process spawn error: ${error.message}`);
        finish({
          status: "failed",
          summary: `Failed to spawn Claude Code CLI: ${error.message}`,
          output: null,
          artifactIds: [],
          error: {
      ...
      child.on("close", (exitCode: number | null) => {
        ...
        if (cancelled) {
          finish({
            status: "failed",
            summary: "Claude Code CLI execution was cancelled",
            output: null,
            artifactIds: [],
            error: {
        ...
        if (timedOut) {
          finish({
            status: "failed",
            summary: `Claude Code CLI execution exceeded timeout of ${timeoutMs}ms`,
            output: null,
            artifactIds: [],
            error: {
              code: "CLAUDE_TIMEOUT",
        ...
        if (stdoutTruncated || stderrTruncated) {
          finish({
            status: "failed",
            summary: "Claude output exceeded maximum configured byte limit",
            output: null,
            artifactIds: [],
            error: {
              code: "CLAUDE_OUTPUT_LIMIT",
        ...
        if (exitCode !== 0) {
          finish({
            ...
            summary:
              stderr.trim() ||
              `Claude Code CLI exited with status ${exitCode ?? "unknown"}`,
            output: null,
            artifactIds: [],
            error: {
        ...
        context.progress(
          "parsing_output",
          85,
          "Parsing Claude structured response",
        );
        ...
        if (!parsed || !parsed.payload) {
          finish({
            status: "failed",
            summary: "Claude did not return a valid structured JSON envelope",
            output: { rawStdout: stdout },
            artifactIds: [],
            error: {
              code: "INVALID_PROTOCOL_OUTPUT",
              message:
                "Unable to parse JSON protocol result from Claude stdout",
              retryable: false,
            },
          });
          return;
        ...
        finish({
          ...
          summary:
            typeof parsed.payload.summary === "string"
              ...
              : `Claude successfully completed '${input.objective}'`,
          ...
          evidence: {
            ...
            logs: [
              `[${startedAt}] Claude spawned (${executable})`,
              `[${completedAt}] Claude finished successfully with exit code 0`,
            ],
          },
        });
      });
      ...
      finish({
        status: "failed",
        summary: `Claude invocation threw exception: ${err instanceof Error ? err.message : String(err)}`,
        output: null,
        artifactIds: [],
        error: {
          code: "CLAUDE_INVOCATION_ERROR",
...
export const claudeWorkerDefinition: WorkerPluginDefinition =
  defineWorkerPlugin({
    manifest: claudeWorkerManifest,
    execute: executeClaudeWorker,
  });
