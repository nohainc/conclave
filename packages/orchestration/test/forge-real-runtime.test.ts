...
class ScenarioModel implements ForgeWorker {
  ...
  async complete(request: ModelRequest): Promise<ModelResponse> {
    ...
    if (this.worker.id === "lead" && role === "Researcher") {
    ...
    } else if (this.worker.id === "implementer") {
    ...
    } else if (this.worker.id === "reviewer" && role === "Verifier") {
    ...
    } else if (this.worker.id === "reviewer") {
      ...
      throw new Error(
        `Unexpected request for ${this.worker.id}: ${role} ${objective}`,
      );
    }

    const text = envelope(request, this.worker.id, messageType, payload);
    return {
      providerRequestId: `${this.worker.id}-${this.requests.length}`,
      text,
