...
export function validateV6UsageDimensions(input: V6UsageDimensions): void {
  ...
  for (const [label, value] of Object.entries(input)) {
    if ((label.endsWith("Tokens") || label.endsWith("Micros") || label.endsWith("Ms")) &&
...
export function computeV6ObservabilityMetrics(
  ...
  const recoverySamples = samples.filter((sample) => sample.checkoutRecoveryAttempted);
  ...
  return {
    queueWaitMs: samples.reduce((sum, sample) => sum + elapsed(sample.queueEnteredAt, sample.startedAt), 0),
    statefulDurationMs: samples.reduce((sum, sample) => sum + elapsed(sample.startedAt, sample.finishedAt), 0),
    checkoutRecoveryRate: recoverySamples.length === 0 ? 0 : recoverySamples.filter((sample) => sample.checkoutRecoverySucceeded).length / recoverySamples.length,
    failedRollbackRate: rollbackSamples.length === 0 ? 0 : rollbackSamples.filter((sample) => !sample.rollbackSucceeded).length / rollbackSamples.length,
    workspaceUtilization: samples.length === 0 ? 0 : activeSamples.length / samples.length,
  };
}
