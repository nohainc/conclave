...
export async function recordV5AssignmentUsage(
  ...
  const assignment = await db.prepare(
    `SELECT wa.id, wa.project_id, wa.run_id, wa.workstream_id, wa.work_request_id,
            ...
            aa.provider_metadata_json, wa.model
       FROM worker_assignments wa
       JOIN execution_workspaces ew ON ew.id = wa.execution_workspace_id
       JOIN ai_accounts aa ON aa.id = wa.account_id
      WHERE wa.id = ?1`,
  ).bind(input.assignmentId).first<Row>();
  if (!assignment) return;

  const providerMetadata = (() => {
    try {
      const parsed = JSON.parse(String(assignment.provider_metadata_json ?? "{}"));
      return parsed && typeof parsed === "object" ? parsed as Row : {};
    } catch {
      return {};
    }
  })();
  const provider = input.provider ?? String(providerMetadata.provider ?? providerMetadata.providerId ?? "unknown");
  const model = input.model ?? (assignment.model == null ? null : String(assignment.model));
...
export async function assertV5BudgetAvailable(
  ...
  ).bind(input.projectId, input.runId, input.accountId).all<Row>();
  const nextInput = integer(input.inputTokens);
  const nextOutput = integer(input.outputTokens);
  for (const budget of budgets.results ?? []) {
    if (budget.max_input_tokens != null && integer(budget.used_input) + nextInput > integer(budget.max_input_tokens)) {
      throw new Error(`Budget ${String(budget.id)} rejects input token usage`);
    }
    if (budget.max_output_tokens != null && integer(budget.used_output) + nextOutput > integer(budget.max_output_tokens)) {
      throw new Error(`Budget ${String(budget.id)} rejects output token usage`);
    }
    if (budget.max_cost_micros != null) {
      if (input.estimatedCostMicros == null) throw new Error(`Budget ${String(budget.id)} requires known monetary cost`);
      if (integer(budget.used_cost) + integer(input.estimatedCostMicros) > integer(budget.max_cost_micros)) {
