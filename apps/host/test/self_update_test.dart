...
void main() {
  test('accepts a Cloud release announcement without applying it', () async {
    ...
    final statuses = <HostUpdateStatus>[];
        ...
        request.response
          ...
          ..write(jsonEncode({
            ...
            'release': {
              ...
              'packageR2Key': 'host/5.0.0.tar.gz',
              'publisher': 'conclave',
              'signature': 'sig-test',
            },
          }));
  ...
  test('does not activate a release without a restart bootstrap', () async {
    ...
    try {
      ...
      expect(await File('${root.path}/host.active').exists(), isFalse);
    } finally {
      await subscription.cancel();
      await server.close(force: true);
      await root.delete(recursive: true);
  ...
  test('activates a verified release and records metadata', () async {
    final root = await Directory.systemTemp.createTemp('conclave-update-');
    final bytes = [1, 2, 3];
    await HostUpdater(root, requireSignature: false).apply(
      ReleasePackage(
          version: '1.1.0',
          channel: 'stable',
          bytes: bytes,
    ...
    expect(await File('${root.path}/host.active').exists(), isTrue);
  ...
  test('restores previous release after a failed health check', () async {
    ...
    final old = File('${root.path}/host.active')..writeAsBytesSync([9]);
    final bytes = [1, 2, 3];
    await expectLater(
      HostUpdater(root, requireSignature: false).apply(
        ReleasePackage(
            version: '2.0.0',
            channel: 'beta',
            bytes: bytes,
  ...
  test('rejects a release requiring a newer protocol', () async {
    ...
    final bytes = [7, 8, 9];
    await expectLater(
      HostUpdater(root, currentProtocolVersion: '2.0', requireSignature: false)
          .apply(
        ReleasePackage(
          version: '2.0.0',
          channel: 'stable',
  ...
  test('rejects a signed or valid release downgrade', () async {
    ...
    expect(await File('${root.path}/host.active').readAsBytes(), currentBytes);
    await root.delete(recursive: true);
  });
  ...
  test('keeps release versions immutable and records provenance', () async {
    ...
    final package = ReleasePackage(
      ...
      packageUrl: 'https://example.test/host.tgz',
    );
    await HostUpdater(root, requireSignature: false)
