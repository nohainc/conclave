...
void main() {
  test(
    ...
    final adapterSource = File('${source.path}/bin/adapter.dart');
    await adapterSource.writeAsString(r'''
import 'dart:convert';
import 'dart:io';
  ...
  await for (final line in stdin.transform(utf8.decoder).transform(const LineSplitter())) {
    ...
    switch (request['type']) {
      ...
      case 'execute.request':
        ...
        await File('${cwd}${Platform.pathSeparator}fixture.txt').writeAsString('written-by-adapter');
        stdout.writeln(jsonEncode({...base, 'type': 'progress', 'assignmentId': request['assignmentId'], 'message': 'fixture running', 'percentage': 50}));
        stdout.writeln(jsonEncode({...base, 'type': 'result', 'assignmentId': request['assignmentId'], 'output': jsonEncode({'cwd': cwd, 'file': await File('${cwd}${Platform.pathSeparator}fixture.txt').readAsString()})}));
        break;
    }
  }
}
''');
    ...
    expect(
        await File('$expectedCwd${Platform.pathSeparator}fixture.txt')
            .readAsString(),
        'written-by-adapter');
