...
const checks = [
  ["Flutter active Workspace selector", "apps/app/lib/src/studio/studio_app.dart", /_workspaceSelector|selectedWorkspaceId|activeWorkspaceId/],
  ["Flutter Workspace settings screen", "apps/app/lib/src/features/workspace/workspace_settings_page.dart", /.*/],
...
for (const [label, file, pattern] of checks) {
  ...
  const activeSource = source.replace(/\/\*[\s\S]*?\*\//g, "").replace(/\/\/.*$/gm, "");
  if (pattern.test(activeSource)) failures.push(`${label}: forbidden active reference in ${file}`);
}
