...
run("Solo, team, and recovery acceptance", "npx", [
  "vitest",
  "run",
...
if (process.env.V6_SKIP_FLUTTER === "1") {
  ...
  run(
    ...
    flutter,
    ["test", "test/projects_pages_test.dart"],
    join(root, "apps/app"),
  );
  run(
    ...
    [
      "test",
      "test/workstream_checkout_manager_test.dart",
      "test/runtime_capabilities_test.dart",
      "test/trust_policy_test.dart",
    ],
    join(root, "apps/host"),
  );
}
...
if (
  process.env.V6_REQUIRE_PLATFORM_MATRIX === "1" &&
  process.env.V6_PLATFORM_MATRIX !== "passed"
) {
  failures.push(
    "Windows/Linux compile matrix is required; run it in CI and set V6_PLATFORM_MATRIX=passed",
  );
} else {
  console.warn(
