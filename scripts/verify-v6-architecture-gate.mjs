...
const forbidden = [
  [
    ...
    /\bWorkspaceRole\b|WORKSPACE_ROLE_PERMISSIONS/,
  ],
  [
    "Host Workspace binding",
    /host_workspace_bindings|host\.bind_workspace|x-conclave-workspace-id/,
  ],
  [
    "configured Worker fallback",
    /configuredWorker|configured_worker|legacy assignment fallback/,
  ],
  [
    "Chat execution authority",
    /startChatExecution|automatic(?:ally)?\s+(?:create|start)\s+(?:a\s+)?(?:Goal|Run)/i,
  ],
  [
    "mutable registered repository path",
    /registered(?:Base)?RepositoryPath|baseRepositoryPath/,
  ],
  [
    "user-supplied worktree path",
    /(?:worktreePath|worktree_path|absoluteWorktreePath)/,
  ],
];
...
function files(entry) {
  ...
  if (stat.isDirectory())
    return fs
      .readdirSync(entry)
      .flatMap((name) => files(path.join(entry, name)));
  return /\.(?:ts|tsx|dart|js|mjs)$/.test(entry) &&
...
const violations = [];
for (const rootEntry of roots.flatMap((entry) =>
  ...
  for (const [label, pattern] of forbidden) {
    if (pattern.test(source))
      violations.push(`${label}: ${path.relative(root, rootEntry)}`);
  }
}
