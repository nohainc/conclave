...
2. create/configure a Worker and complete its local credential setup;
...
The execution bridge required by this path is defined by the v6 execution
foundation and configured Worker migrations: a Worker Type, a configured Worker,
its Workspace binding and local readiness, Workspace Project Grants, and the
immutable assignment requester snapshot. The acceptance deliberately creates
no AI Account, Project Account Grant, or legacy package-installation record.
It is a clean v6 migration and introduces no v4 compatibility objects.
